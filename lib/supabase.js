...
export const userRolesService = {
  ...
  async getAllUsers() {
    try {
      ...
      console.log('Récupération de tous les utilisateurs depuis la base de données...');
      
      // Récupérer d'abord tous les utilisateurs depuis la table users
      const { data: usersData, error: usersError } = await supabase
        .from('users')
        .select('*')
        .order('created_at', { ascending: false });

      console.log('Users Data from DB:', usersData);
      console.log('Users Error from DB:', usersError);

      if (usersError) {
        console.error('Erreur détaillée users:', usersError);
        throw usersError;
      }
      ...
      if (rolesError) {
        console.error('Erreur détaillée roles:', rolesError);
        throw rolesError;
      }

      // Créer une map des rôles par user_id
      const rolesMap = {};
      ...
      // Combiner les données des utilisateurs et des rôles
      let users = [];
      
      if (usersData && usersData.length > 0) {
        console.log('Utilisateurs trouvés dans table users:', usersData.length);
        // Utiliser les utilisateurs de la table users
        users = usersData.map(user => {
          ...
          const userData = {
            ...
            updated_at: role.updated_at || user.updated_at,
            last_sign_in_at: user.last_sign_in_at
          };

          console.log('Utilisateur traité:', userData);
          return userData;
        });
      } else {
        console.log('Table users vide, tentative avec auth.users...');
        // Fallback: essayer avec auth.users si la table users est vide
        try {
          ...
          if (!authError && authUsers && authUsers.users) {
            console.log('Utilisateurs trouvés dans auth.users:', authUsers.users.length);
            users = authUsers.users.map(user => {
              ...
              const userData = {
                user_id: user.id,
                email: user.email,
                name: user.user_metadata?.name || 'Nom non disponible',
                role: role.role,
                status: role.status,
                created_at: role.created_at || user.created_at,
                updated_at: role.updated_at || user.updated_at,
              ...
              console.log('Utilisateur auth traité:', userData);
              return userData;
            });
          } else {
            console.error('Impossible de récupérer les utilisateurs depuis auth.users');
            // Si auth.users ne fonctionne pas, créer une liste avec juste les rôles
            if (rolesData && rolesData.length > 0) {
              console.log('Fallback: utilisation des rôles uniquement');
              users = rolesData.map(role => ({
                user_id: role.user_id,
                email: role.email || 'Email non disponible',
                name: role.name || 'Nom non disponible',
                role: role.role,
                status: role.status,
                created_at: role.created_at,
                updated_at: role.updated_at,
          ...
          console.error('Erreur lors de listUsers:', authListError);
          // Dernier fallback: utiliser uniquement les rôles
          if (rolesData && rolesData.length > 0) {
            console.log('Dernier fallback: utilisation des rôles uniquement');
            users = rolesData.map(role => ({
              user_id: role.user_id,
              email: role.email || 'Email non disponible',
              name: role.name || 'Nom non disponible',
      ...
      console.error('=== FIN ERREUR getAllUsers ===');
      throw error;
    }
  },
